...
window.addEventListener('load', function(e) {  
    ...
    if (defaultBooks !== false) populateStorage();
    render();
  });
...
for (let i=0; i<length; i++){     
    ...
    let readStatus="";
    if (myLibrary[i].check==1) {readStatus="Yes"} else {readStatus="No"};
    ...
    delBut.addEventListener("click", function () {
        ...
        if (myLibrary.length==0) localStorage.setItem('defaultBooks', false);
        localStorage.setItem('myLibrary', JSON.stringify(myLibrary));
        render();
    })
}
